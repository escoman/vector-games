...
/* Регистр строки (скролл, порт 3). Пишет ТОЛЬКО переменную gfx_scroll_row:
 * в порт значение выпишет кадровый опрос клавиатуры (kbd_scan_now в ISR /
 * kbd_scan_rows в цикле) на ближайшем VSync, когда луч в гашении.
 * Отложенная запись безопасна из прерывания: немедленный v06_out()
 * содержит di/ei вокруг самомодифицирующегося кода, и его ei внутри ISR
 * разрешает прерывания посреди обработчика (рекурсия on_frame → порча
 * бордюра/PB и лишние drum_tick). Для немедленной записи ВНЕ прерывания
 * пишите порт напрямую: v06_out(V06_PIA_PA, row). */
