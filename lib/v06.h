...
/* ---------------- КР580ВИ53: аппаратный API (vi53/vi53.c) ------------- */

/* Единственный владелец записи в порты ВИ53 (ТЗ §16). Доступно любому
 * ROM независимо от MUSIC_ONLY. channel: 0/1/2 (порты 0x0B/0x0A/0x09);
 * divisor: частота = 1500000 / divisor (режим 3), 0 = тишина.
 *   vi53_set_channel()    — 0 = только управляющее слово режима 3,
 *                           счётчик НЕ загружается (способ music.c);
 *   vi53_set_channel_m0() — 0 = режим 0 (OUT=0) + два нулевых байта
 *                           (способ sound.c). */
...
/* ------------------ AY-3-8910: аппаратный API (ay/ay.c) --------------- */

/* Единственный владелец записи в регистры AY из C (ТЗ §17). Порты:
 * выбор регистра — НЕЧЁТНЫЙ 0x15, данные — 0x14. ROM, линкующий ay.c,
 * обязан линковать и drums.asm (там определено зеркало микшера g_ay_r7).
 * Три тональных канала A/B/C: период R0/R1, R2/R3, R4/R5; громкость
 * R8/R9/R10. Огибающая ОДНА ОБЩАЯ на все каналы (R11/R12/R13).
 * Доступно любому ROM независимо от MUSIC_ONLY. */

/* Маски каналов (chan_mask) для ay_set_envelope / ay_set_fixed_volume. */
#define AY_CH_A   0x01    /* канал A (R8)                          */
#define AY_CH_B   0x02    /* канал B (R9)                          */
#define AY_CH_C   0x04    /* канал C (R10); общий с ударными drums.asm */
...
/* Запись одного регистра AY (reg 0..15, val). */
extern void ay_write(unsigned char reg, unsigned char val);
/* Обновить зеркало g_ay_r7 и записать микшер R7. */
extern void ay_set_r7(unsigned char val);
/* Базовая конфигурация микшера: фикс. громкости = 15, R7 = 0xF8
 * (тоны A/B/C вкл, шум ABC выкл). */
extern void ay_mixer_init(void);
/* Установить тон канала ch (0=A/1=B/2=C) по ГОТОВОМУ периоду AY
 * (12 бит, 0 = тишина, громкость в 0). Аппаратный уровень: period →
 * R0..R5 + громкость/огибающая. Музыкальный плеер использует её с
 * периодом из таблицы ay_period_tab[], поэтому в ISR нет 32-битной
 * математики. Применяет огибающую на атаке, если для канала выставлен
 * ay_env_mode (см. ay_set_envelope). */
extern void ay_set_tone_period(unsigned char ch, unsigned int period);
/* То же, но по делителю ВИ53 (конвертируется в период AY 887/12000,
 * 1..4095); 0 = тишина. Обёртка для внешнего API. */
extern void ay_set_tone(unsigned char ch, unsigned int div_VI53);
/* Выключить тональный канал (громкость в 0, период не трогаем). */
extern void ay_note_off(unsigned char ch);
/* Тишина на всех трёх тональных каналах (R8/R9/R10 = 0). */
extern void ay_mute_all(void);

/* Определить аппаратную огибающую и подключить к ней каналы: сохранить
 * глобальные shape/period и выставить ay_env_mode = chan_mask; применить
 * на СЛЕДУЮЩЕЙ атаке ноты (ay_set_tone запрограммирует R11/R12 (period),
 * R13 (shape, перезапуск генератора) и выставит бит 4 в R8/R9/R10
 * подключённых каналов). В регистры сразу НЕ пишет — иначе огибающая
 * подключилась бы к уже звучащему тону и дала непрерывный сигнал.
 * chan_mask: биты AY_CH_A/B/C (0x07 — все три). При chan_mask == 0
 * (после маскирования 0x07) вызов игнорируется.
 *
 * ВНИМАНИЕ: R11/R12/R13 — общие регистры AY-3-8910 на все три канала:
 * shape/period одни на всю маску и применяются ко ВСЕМ подключённым
 * каналам. Независимых огибающих на канал в AY-3-8910 нет. */
extern void ay_set_envelope(unsigned char chan_mask,
                            unsigned char shape,
                            unsigned int period);

/* Вернуть каналы на фиксированную громкость: снять флаг огибающей у
 * каналов chan_mask (биты AY_CH_A/B/C) и запомнить volume (обрезается до
 * 0..15, применяется ко всем каналам маски); применить на СЛЕДУЮЩЕЙ
 * атаке ноты (ay_set_tone: бит 4 сброшен, volume = сохранённое). В
 * R8/R9/R10 сразу НЕ пишет (иначе — фоновый тон). volume = 0 — канал
 * выключен. При chan_mask == 0 вызов игнорируется. */
...
/* Явная привязка устройства вывода (ТЗ §6/§14 — никакого глобального
 * «режима звука»). music_start_vi53/music_start_ay сбрасывают транспорт
 * и привязывают вывод; music_use_vi53/music_use_ay перепривязывают вывод
 * БЕЗ сброса позиции и состояния (во время воспроизведения), заменяя
 * прежний глобальный переключатель режима. Три мелодических канала: 0→VI53 CH0/AY A,
 * 1→CH1/AY B, 2→CH2/AY C. Ударные (шум) следуют выбранному устройству:
 * ВИ53 → Tape Out PC0 (LFSR в drums.asm), AY → шумовой канал AY (Noise C);
 * маршрут выставляется внутри этих функций через drum_route_tape/ay(). */
...
/* Синтезатор шума: физический выход выбирается модульным маршрутом
 * (drum_route в drums.asm), а не глобальным «режимом звука»:
 *   drum_route_tape() — программный 1-битный LFSR-шум на Tape Out
 *     (PIA1 Port C bit 0): R6 задаёт число сдвигов за тик, R10 —
 *     duty-окно переключений PC0 (маршрут по умолчанию);
 *   drum_route_ay()   — канал C AY-3-8910 в «tone off, noise on»
 *     (R6 период, R10 громкость, микшер R7).
 * Переключение маршрута глушит звучащий удар.
...
extern void drum_route_tape(void);      /* шум на Tape Out (PC0), умолч. */
