 ...
 * как в v06_out из v06io.asm) под di/ei. Поэтому кадровое прерывание не
 * может подменить порт, а запись атомарна — функции безопасны и в ISR.
 *
 ...
 * volatile-переменную, а out (n),a с немедленным адресом выполняется под
 * di/ei — атомарно относительно кадрового прерывания. Каждый порт имеет
 * свой немедленный адрес, поэтому четыре отдельных helper'а (аналог четырёх
 * PUBLIC-функций прежнего vi53out.asm). */
