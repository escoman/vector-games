 ...
 * физически единственные, поэтому в ay_set_envelope передаётся МАСКА
 * подключаемых каналов (chan_mask), а не номер канала: форма/период одни
 * на всю маску. Подключение канала — бит 4 в R8/R9/R10; каналы, которым
 * бит 4 не выставлен, остаются на фиксированной громкости. */

/* Маски каналов для ay_set_envelope (chan_mask). */
 ...
 * chan_mask: биты AY_CH_A/B/C (0x07 — все три). Возврат — ничего; при
 * chan_mask == 0 (после маскирования 0x07) вызов игнорируется.
 *
 ...
 * каналам. Пример: ay_set_envelope(AY_CH_A, DECAY, 1000), затем
 * ay_set_envelope(AY_CH_A | AY_CH_B, TRIANGLE, 4000) — и A, и B теперь на
 * TRIANGLE/4000. Независимых огибающих на канал в AY-3-8910 нет. */
 ...
 * выключен. При chan_mask == 0 (после маскирования 0x07) вызов
 * игнорируется. */
